...
*Test Assumptions:* 
1. Metric variables
2. Random variables are independent and identically distributed yes (assuming same data collection and data transformation procedures), explain
...
gghistogram(high_gdp_cap, main = 'Distribution of Life Ladder Scores in High GDP per Capita Countries', x = 'Life.Ladder', fill= 'steelblue', bins = 5,  xlab = 'Life Ladder Scores', ylab = 'Count')
```

```{r, echo=FALSE, comment=""}
#hist($) #fix #make pretty 
gghistogram(low_gdp_cap, main = 'Distribution of Life Ladder Scores in Low GDP per Capita Countries', x = 'Life.Ladder', fill= 'steelblue', bins = 5,  xlab = 'Life Ladder Scores', ylab = 'Count')
```

```{r, echo=FALSE, comment=""}
# Can do a Shapiro test as an extra source of evidence to test for normalcy
shapiro.test(wh_data$Life.Ladder)
```

```{r, comment=""}
shapiro.test(wh_data$Log.GDP.per.capita)
#https://www.datanovia.com/en/lessons/normality-test-in-r/#check-normality-in-r 

```
The p value for the above is les 
