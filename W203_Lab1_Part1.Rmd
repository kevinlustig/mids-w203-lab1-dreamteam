...
DOUBLE CHECK THIS PP Validating the independence of the random samples seems hazardous assuming that countries and people might be linked somehow. In other words, knowing some information about one country might provide information/insights about neighboring countries. The second assumption is not completely verified.

Assuming that data collection and transformation protocols were followed in similar fashion throughout the data collection and transformation activities, it can be assumed that the random variables are independant and identically distributed. 
...
```{r, echo=FALSE, comment="", fig.width=4, fig.height=2, fig.align='center', include=FALSE}
...
```
The Shapiro Wilk test for the low GDP per capita country distribution (above) has a p-value of 0.0001319 which is less than 0.05, indicating that the sample is not normal.
...
```
The Shapiro Wilk test for the GDP per capita of high countries has a p-value of 0.00764, indicating this sample is not normal.
...
**Conduct the test?**\newline
NO: Considering that the normality assumption is not met, the Two Sample t-Test would not be best suited for this context.
...
The ages of the legislators come from the same distribution (politicians' ages) is also independent and identically distributed. Moreover, a politician's age does not provide information about the age of any other politician, which satisfies the independence condition and thus the IID, satisfying the second assumption. 

The third assumption regarding the shape and spread of the of the two-party samples of the legislator ages. A summary of their distributions and visual aids of their sample distributions (histograms and box and whisker plot) will help determine whether this assumption is satisfied. 
```{r, echo=FALSE, comment=""}
# Load in Data set 
...
```
Looking closer at the sample distributions in the above figures, it seems that while the distributions may not FINISH 


**Conduct the test?**\newline
CHECK Considering these elements, the Wilcoxon Rank Sum test is applicable.
...
boxplot( wine_data$difference, data = wine_data,
        ...
        horizontal = TRUE)
```
The distribution of the difference between the paired samples  "heart" and "liver" is slightly left skewed. Moreover the tight and left heights of the density curve at a similar distance to the median are not equivalent. This distribution is not perfectly symmetric around some median. However, we should not expect the differences in distribution to be perfectly symmetric especially when the sample size is small (n=21). Thus, the assumption of symmetry is difficult to assess here but could be considered as verified.
