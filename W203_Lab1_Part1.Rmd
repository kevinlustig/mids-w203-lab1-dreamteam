...
The "Life Ladder" score (LLS) variable is composed of continuous values. Data can be classified and distance between values make sense. This point is important considering that the Two-Sample t-Test aims at comparing means of two random variables.
...
Density plots are used below to better assess the correlation between the samples and the normal distribution. Black dotes closely positioned to the 45 degrees reference line would suggest a data distribution close to the normal distribution.
...
The positions of black dotes fluctuates around the reference line without forming a straight line. The visual study of distribution does not provide clear yes/no answer.
...
Additionally, the values of both observations can be compared,ranked as they share the same zero-base reference and counting granularity. Both rv are measured on a similar metric scale. The first assumption is verified.
